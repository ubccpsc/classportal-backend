...
then
  ...
  printf "docker-build-helper.sh:: Building Docker image for Deliverable ${deliverable}: ${tagName}\n\n"
else
  tagName=autotest/cpsc${courseId}__bootstrap:master
  printf "docker-build-helper.sh:: Building Docker image for Course ${courseId}: ${tagName}\n\n"
fi
