...
let createTeam = function(course_Id: Object, req: any) {
  ...
  for ( let i = 0; i < newTeamMembers.length; i++) {
    newTeam.members.push(newTeamMembers[i]);
  }

  return Team.create(newTeam).catch(err => logger.info('Error creating course: ' + err));
};
